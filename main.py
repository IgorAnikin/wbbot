...
FAL_KEY         = os.getenv("FAL_KEY", "")  # добавьте в Railway
...
# ---------- Fal.ai ----------
FAL_IMG2IMG_URL = "https://fal.run/fal-ai/flux-pro/v1.1/img2img"
...
def presets_for(mode: str) -> tuple[str, int]:
    ...
    if mode == "set":
        ...
        return prompt, 12
    # review (пока один кадр-заглушка)
    prompt = (
...
async def fal_generate(image_url: str, mode: str) -> list[str]:
    ...
    async with httpx.AsyncClient(timeout=300) as client:
        resp = await client.post(FAL_IMG2IMG_URL, headers=headers, json=payload)
        # Fal может вернуть 402 (лимит/оплата), 422 (неверные поля)
...
async def on_photo(msg: Message):
    try:
        ...
        await msg.answer(f"⚠️ Fal.ai вернул ошибку {e.response.status_code}: {e.response.text[:400]}")
